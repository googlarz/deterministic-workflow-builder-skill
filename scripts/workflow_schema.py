...
def _add_issue(
    issues: list[Issue], severity: str, path: Path, message: str, line: int | None = None
) -> None:
...
def _validate_contract_entry(
    ...
    if contract is None:
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` field `{field_name}` must contain strings or objects.",
        )
        return
    ...
    if contract_type not in VALID_CONTRACT_TYPES:
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` has invalid contract type `{contract_type}` in `{field_name}`.",
        )
    ...
    if not isinstance(path_value, str) or not path_value:
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` contract in `{field_name}` must define non-empty `path`.",
        )
        return
    ...
    if contract_path.is_absolute():
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` contract path `{path_value}` must be relative.",
        )
    if ".." in contract_path.parts:
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` contract path `{path_value}` must not contain `..`.",
        )
    if workflow_dir is not None and contract.get("required", True):
        ...
        if workflow_dir.resolve() not in (resolved, *resolved.parents):
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Step `{step_id}` contract path `{path_value}` escapes the workflow directory.",
            )
    if "sha256" in contract and (
        not isinstance(contract["sha256"], str) or len(contract["sha256"]) != 64
    ):
        _add_issue(
            issues,
            "error",
            manifest_path,
            f"Step `{step_id}` contract path `{path_value}` has invalid `sha256`.",
        )
    for size_field in ("min_size_bytes", "max_size_bytes"):
        if size_field in contract and (
            not isinstance(contract[size_field], int)
            or isinstance(contract[size_field], bool)
            or contract[size_field] < 0
        ):
            _add_issue(
                issues,
                "error",
                manifest_path,
                f"Step `{step_id}` contract path `{path_value}` has invalid `{size_field}`.",
            )
    if "retention" in contract and not isinstance(contract["retention"], dict):
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` contract path `{path_value}` must use object `retention` metadata.",
        )
    if (
        contract_type == "json"
        and "schema" in contract
        and not isinstance(contract["schema"], dict)
    ):
        _add_issue(
            issues,
            "error",
            manifest_path,
            f"Step `{step_id}` JSON contract `{path_value}` must use object `schema`.",
        )
...
def _validate_validation_check(
    ...
    if not isinstance(entry, dict):
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` validation checks must be strings or objects.",
        )
        return
    ...
    if check_type not in VALID_VALIDATION_TYPES:
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` has invalid validation check type `{check_type}`.",
        )
        return
    if check_type in {"file_exists", "path_absent", "json_required_keys"}:
        if not isinstance(entry.get("path"), str) or not entry["path"]:
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Step `{step_id}` validation check `{check_type}` must define `path`.",
            )
    if check_type == "json_required_keys":
        ...
        if not isinstance(keys, list) or not all(isinstance(item, str) and item for item in keys):
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Step `{step_id}` json_required_keys must define string `required_keys`.",
            )
    if check_type == "log_contains" and (
        ...
        _add_issue(
            ...
            "error",
            manifest_path,
            f"Step `{step_id}` log_contains check must define non-empty `value`.",
        )
    if check_type == "command" and (
        not isinstance(entry.get("command"), str) or not entry["command"]
    ):
        _add_issue(
            issues,
            "error",
            manifest_path,
            f"Step `{step_id}` command validation must define non-empty `command`.",
        )


def validate_manifest(
    manifest: dict[str, Any], manifest_path: Path, workflow_dir: Path | None = None
) -> list[Issue]:
    issues: list[Issue] = []
            ...
            if isinstance(environment, dict):
                ...
                if not isinstance(network_mode, str) or not network_mode:
                    _add_issue(
                        ...
                        "error",
                        manifest_path,
                        "`environment.network_mode` must be a non-empty string.",
                    )
            tooling = _expect_type(issues, manifest_path, manifest, "tooling", dict)
            if isinstance(tooling, dict):
                if "allowlisted_commands" in tooling and not isinstance(
                    tooling["allowlisted_commands"], list
                ):
                    _add_issue(
                        issues,
                        "error",
                        manifest_path,
                        "`tooling.allowlisted_commands` must be a list.",
                    )
            migrations = _expect_type(issues, manifest_path, manifest, "migrations", dict)
            if isinstance(migrations, dict):
                current_from = migrations.get("current_from")
                if current_from is not None and (
                    not isinstance(current_from, int) or isinstance(current_from, bool)
                ):
                    _add_issue(
                        issues,
                        "error",
                        manifest_path,
                        "`migrations.current_from` must be an integer when provided.",
                    )
    ...
    if isinstance(failure_policy, dict):
        ...
        if not isinstance(retries, int) or isinstance(retries, bool) or retries < 0:
            _add_issue(
                ...
                "error",
                manifest_path,
                "`failure_policy.max_retries` must be a non-negative integer.",
            )
    ...
    if isinstance(audit, dict):
        ...
        if not isinstance(directory, str) or not directory:
            _add_issue(
                issues, "error", manifest_path, "`audit.directory` must be a non-empty string."
            )

    residual = _expect_type(
        issues, manifest_path, manifest, "residual_nondeterminism", list, allow_empty=False
    )
        ...
        for item in residual:
            if not isinstance(item, str) or not item.strip():
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    "`residual_nondeterminism` entries must be non-empty strings.",
                )
        ...
        for index, step in enumerate(steps, start=1):
            ...
            for field in STEP_REQUIRED_FIELDS:
                if field not in step:
                    _add_issue(
                        issues, "error", manifest_path, f"Step #{index} is missing `{field}`."
                    )
            step_id = step.get("id")
            if isinstance(step_id, str):
                ...
                _add_issue(
                    issues, "error", manifest_path, f"Step #{index} must have a string `id`."
                )
            ...
            if not isinstance(step.get("requires_approval"), bool):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` must have boolean `requires_approval`.",
                )
            ...
            if not isinstance(retry_limit, int) or isinstance(retry_limit, bool) or retry_limit < 0:
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` must have non-negative integer `retry_limit`.",
                )
            ...
            if not isinstance(depends_on, list):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` field `depends_on` must be a list.",
                )
            if step_type in SCRIPTED_STEP_TYPES:
                ...
                if not isinstance(script, str) or not script:
                    _add_issue(
                        ...
                        "error",
                        manifest_path,
                        f"Step `{step_id}` must define non-empty `script`.",
                    )
                    ...
                    if not script_path.exists():
                        _add_issue(
                            ...
                            "error",
                            script_path,
                            f"Step script referenced by `{step_id}` does not exist.",
                        )
            ...
            if not isinstance(produces, list):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` field `produces` must be a list.",
                )
            else:
                for entry in produces:
                    _validate_contract_entry(
                        issues, manifest_path, workflow_dir, step_id, "produces", entry
                    )
            if not isinstance(consumes, list):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` field `consumes` must be a list.",
                )
            else:
                for entry in consumes:
                    _validate_contract_entry(
                        issues, manifest_path, workflow_dir, step_id, "consumes", entry
                    )
            if not isinstance(commands, list):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` field `commands` must be a list.",
                )
            if not isinstance(validation_checks, list):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` field `validation_checks` must be a list.",
                )
            else:
            ...
            if "executor_config" in step and not isinstance(step["executor_config"], dict):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` must use object `executor_config`.",
                )
            if "rollback" in step:
                ...
                if not isinstance(rollback, dict):
                    _add_issue(
                        ...
                        "error",
                        manifest_path,
                        f"Step `{step_id}` rollback must be an object.",
                    )
                else:
                    ...
                    if not isinstance(script, str) or not script:
                        _add_issue(
                            ...
                            "error",
                            manifest_path,
                            f"Step `{step_id}` rollback must define non-empty `script`.",
                        )
                        ...
                        _add_issue(
                            ...
                            "error",
                            workflow_dir / script,
                            f"Rollback script for `{step_id}` does not exist.",
                        )
                    ...
                    if when not in {"manual", "on_failure"}:
                        _add_issue(
                            ...
                            "error",
                            manifest_path,
                            f"Step `{step_id}` rollback.when must be `manual` or `on_failure`.",
                        )
                    ...
                    if not isinstance(preconditions, list):
                        _add_issue(
                            ...
                            "error",
                            manifest_path,
                            f"Step `{step_id}` rollback.preconditions must be a list.",
                        )
            if "working_directory" in step and not isinstance(step["working_directory"], str):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Step `{step_id}` working_directory must be a string.",
                )
        ...
        if isinstance(graph, dict) and graph.get("execution_model") == "dag":
            ...
            if len(ordered) != len(step_map):
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    "Workflow DAG contains a cycle or unreachable dependency chain.",
                )
    ...
    for index, sidecar in enumerate(sidecars, start=1):
        ...
        for field in SIDECAR_REQUIRED_FIELDS:
            if field not in sidecar:
                _add_issue(
                    issues, "error", manifest_path, f"Sidecar #{index} is missing `{field}`."
                )
        ...
        if kind not in VALID_SIDECAR_KINDS:
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Sidecar `{sidecar_id}` has invalid `kind` `{kind}`.",
            )
        ...
        if isinstance(consumer_step, str) and consumer_step not in step_ids:
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Sidecar `{sidecar_id}` references unknown `consumer_step` `{consumer_step}`.",
            )
        ...
        if not isinstance(containment, dict):
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Sidecar `{sidecar_id}` must contain a `containment` object.",
            )
        else:
            for field in ("mode", "enforced_by", "notes"):
                if (
                    field not in containment
                    or not isinstance(containment.get(field), str)
                    or not containment.get(field)
                ):
                    _add_issue(
                        issues,
                        "error",
                        manifest_path,
                        f"Sidecar `{sidecar_id}` containment must include non-empty `{field}`.",
                    )
        if kind == "prompt":
            ...
            if not isinstance(prompt_asset, str) or not prompt_asset:
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Sidecar `{sidecar_id}` must define `prompt_asset`.",
                )
            elif workflow_dir is not None and not (workflow_dir / prompt_asset).exists():
                _add_issue(
                    issues,
                    "error",
                    workflow_dir / prompt_asset,
                    f"Prompt asset for sidecar `{sidecar_id}` does not exist.",
                )
            ...
            if prompt_sha256 is not None and (
                ...
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Sidecar `{sidecar_id}` has invalid `prompt_sha256`.",
                )
            elif (
                workflow_dir is not None
                and isinstance(prompt_asset, str)
                and isinstance(prompt_sha256, str)
            ):
                actual_sha256 = (
                    __import__("hashlib")
                    .sha256((workflow_dir / prompt_asset).read_bytes())
                    .hexdigest()
                )
                if actual_sha256 != prompt_sha256:
                    _add_issue(
                        ...
                        "error",
                        manifest_path,
                        f"Sidecar `{sidecar_id}` prompt digest does not match `prompt_sha256`.",
                    )
        if kind == "skill":
            ...
            if not isinstance(skill_path, str) or not skill_path:
                _add_issue(
                    ...
                    "error",
                    manifest_path,
                    f"Sidecar `{sidecar_id}` must define `skill_path`.",
                )
        ...
        if output_schema is None or not isinstance(output_schema, dict):
            _add_issue(
                ...
                "error",
                manifest_path,
                f"Sidecar `{sidecar_id}` must define object `output_schema`.",
            )
...
def _topological_step_order(manifest: dict[str, Any]) -> list[str]:
    ...
    ready = sorted(
        ...
        key=lambda item: order_hint[item],
    )
    result: list[str] = []
    queue = deque(ready)
    while queue:
